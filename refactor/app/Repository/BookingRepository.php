...
class BookingRepository extends BaseRepository
{
    ...
    /**
     * Get jobs for the specified user ID.
     *
     * @param $user_id
     * @return array
     */
    ...
    /**
     * Get job history for the specified user ID.
     *
     * @param $user_id
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            $jobs = Job::getTranslatorJobs($cuser->id, 'complete');
            $jobs = $jobs->pluck('jobs')->all();
            $usertype = 'translator';
     ...
     * @return array
     */
    public function store(User $user, array $jobData)
    {
    ...
    /**
     * Store job-related email information.
     *
     ...
     * @return array
     */
    public function storeJobEmail($userType, $userEmail, $jobId, $details)
    {
        $job = Job::find($jobId);
        ...
        $this->mailer->sendJobEmail($userEmail, $job);
    
        return [
            'success' => true,
            'message' => 'Job email stored and sent successfully.',
        ];
    }
    
    /**
     * Convert a job object into an array of data for push notifications.
     *
     * @param Job $job
     * @return array
     */
...
public function getPotentialJobIdsWithUserId($user_id)
{
    ...
    else if ($translator_type == 'courttranslator')
        $job_type = 'court';  /* for court translator only show court jobs. */
    $language = $user_meta->user_language;
    $gender = $user_meta->gender;
    $translator_level = $user_meta->translator_level;
    $jobIds = array();
    if ($job_type == 'unpaid') {
        if ($translator_type == 'rwstranslator') {
            $rws_jobs = RwsJob::whereIn('translator_gender', array('a', $gender))
                ->where('status', 'pending')
                ->where('translator_level', '>=', $translator_level)
                ->get();
            foreach ($rws_jobs as $rws_job) {
                $jobIds[] = $rws_job->job_id;
            }
        } else if ($translator_type == 'courttranslator') {
            $court_jobs = CourtJob::whereIn('translator_gender', array('a', $gender))
                ->where('status', 'pending')
                ->where('translator_level', '>=', $translator_level)
                ->get();
            foreach ($court_jobs as $court_job) {
                $jobIds[] = $court_job->job_id;
            }
        } else {
            $jobs = Job::whereIn('user_gender', array('a', $gender))
                ->whereIn('user_language', array($language, 'a'))
                ->where('status', 'pending')
                ->where('translator_level', '>=', $translator_level)
                ->get();
            foreach ($jobs as $job) {
    ...
    } else if ($job_type == 'paid') {
        $jobs = Job::whereIn('user_gender', array('a', $gender))
            ->whereIn('user_language', array($language, 'a'))
            ->where('status', 'assigned')
            ->where('translator_id', $user_id)
            ->get();
        foreach ($jobs as $job) {
            $jobIds[] = $job->id;
        }
...
{
    $user_meta = UserMeta::where('user_id', $exclude_user_id)->first();
    $user_level = $user_meta->translator_level;

    $potential_translators = $this->getPotentialTranslators($job);

    $potential_translators = $potential_translators->filter(function ($value, $key) use ($user_level) {
        if ($value['translator_level'] >= $user_level)
            return $value;
    });

    $potential_translators = $potential_translators->filter(function ($value, $key) {
        if ($value['is_busy'] == 0)
            return $value;
    });
    ...
    $potential_translators = $potential_translators->filter(function ($value, $key) {
        if ($value['is_push_notify'] == 1)
            return $value;
    });

    $potential_translators = $potential_translators->filter(function ($value, $key) use ($job) {
        if (strtotime($value['start_time']) <= strtotime($job['session_time']) && strtotime($value['end_time']) >= strtotime($job['session_time']))
            return $value;
    });

    $potential_translators->sortBy('translator_level', SORT_NUMERIC)->values();

    foreach ($potential_translators as $potential_translator) {
        $this->sendJobNotification($potential_translator['user_id'], $job, $data);
    }
}
